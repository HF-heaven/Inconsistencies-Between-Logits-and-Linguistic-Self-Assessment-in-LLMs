...
generation_tokenizer = AutoTokenizer.from_pretrained(args.generation_model, use_fast=False, cache_dir=config.data_dir)
...
for sample in tqdm(sequences):
    ...
    cleaned_generations = torch.full_like(
        sample['generations'], 
    ...
    sample['confidence'] = confidence_list

    most_likely_generation_confidence = extract_confidence(most_likely_generation)
    second_most_likely_generation_confidence = extract_confidence(second_most_likely_generation)
    sample['most_likely_generation_confidence'] = most_likely_generation_confidence
    sample['second_most_likely_generation_confidence'] = second_most_likely_generation_confidence
    for string in strings_to_filter_on:
        if string in most_likely_generation:
            most_likely_generation = most_likely_generation.split(string)[0]
        if string in second_most_likely_generation:
            second_most_likely_generation = second_most_likely_generation.split(string)[0]
    clean_most_likely_ids = torch.cat(
        [sample['prompt'].to(device),
         torch.tensor(tokenizer(most_likely_generation)['input_ids'][1:], device=device)])
    clean_second_most_likely_ids = torch.cat(
        [sample['prompt'].to(device),
         torch.tensor(tokenizer(second_most_likely_generation)['input_ids'][1:], device=device)])
    sample['most_likely_generation'] = most_likely_generation
    sample['second_most_likely_generation'] = second_most_likely_generation
    sample['most_likely_generation_ids'] = clean_most_likely_ids[:min(len(clean_most_likely_ids), max_len_of_generations)]
    sample['second_most_likely_generation_ids'] = clean_second_most_likely_ids[:min(len(clean_second_most_likely_ids), max_len_of_generations)]
