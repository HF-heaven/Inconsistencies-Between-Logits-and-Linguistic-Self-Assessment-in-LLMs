...
tokenizer = AutoTokenizer.from_pretrained(args.model, use_fast=False, cache_dir=config.data_dir)
...
if not pathlib.Path(f'{config.data_dir}/trivia_qa_{args.model.split['/'][0]}').exists():
    ...
    for sample in data_for_few_shot_prompt:
        ...
        few_shot_prompt += f"Question: {sample['question']} Answer: {sample['answer']['value']} Confidence: {conf}.\n"
    ...
    val_data.save_to_disk(f'{config.data_dir}/trivia_qa_{args.model.split['/'][0]}')
else:

    val_data = datasets.load_from_disk(f'{config.data_dir}/trivia_qa_{args.model.split['/'][0]}')
