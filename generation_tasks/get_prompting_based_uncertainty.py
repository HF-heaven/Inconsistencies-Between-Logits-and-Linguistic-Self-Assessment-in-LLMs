...
generation_tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=False, cache_dir=config.data_dir)
...
with open(f'{config.output_dir}/sequences/{run_name}/{model_name.split("/")[-1]}_generations.pkl', 'rb') as infile:
...
for sequence in subset_of_sequences_for_few_shot_prompt:
    ...
    correct = ' True' if sequence['rougeL_to_target'] > 0.03 else ' False'
...
with torch.no_grad():
    ...
    for sequence in tqdm(sequences_for_few_shot_prompt[:n_samples_to_use]):
        ...
        generated_texts = '\n'.join(sequence['cleaned_generated_texts'][:number_of_few_shot_samples])
        most_likely_answer = sequence['most_likely_generation']
        # print("rougeL_to_target", sequence['rougeL_to_target'])
        correct = 1.0 if sequence['rougeL_to_target'] > 0.03 else 0.0
    ...
    print("corrects and probabilities", 1 - torch.tensor(corrects), torch.tensor(p_trues))
    p_true_auroc = roc_auc_score(1 - torch.tensor(corrects), torch.tensor(p_trues))
    print("P(True) AUROC:", p_true_auroc)

    # Store p_true aurocs in a pickle file
    with open(f'{config.output_dir}/sequences/{run_name}/{model_name.split("/")[-1]}_p_true_aurocs.pkl', 'wb') as outfile:
