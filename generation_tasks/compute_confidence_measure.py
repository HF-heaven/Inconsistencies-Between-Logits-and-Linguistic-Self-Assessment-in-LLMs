...
def compute_conf(conf_list):
    ...
    for c in conf_list:
        if c == 'N/A':
            ...
            conf += c
            len_list += 1
    return conf / len_list if len_list > 0 else 'N/A'
...
def get_predictive_entropy_over_concepts(log_likelihoods, semantic_set_ids):
    ...
    for row_index in range(mean_across_models.shape[0]):
        ...
        # aggregated_conf = []
        # conf_row = confidence_list[row_index]
        # print(semantic_set_ids_row)
        # print(confidence_list[row_index])
        for semantic_set_id in torch.unique(semantic_set_ids_row):
            ...
            # aggregated_conf.append(compute_conf([conf_row[i] for i in range(len(conf_row)) if semantic_set_ids_row[i] == semantic_set_id]))
            # print(semantic_set_id)
        aggregated_likelihoods = torch.tensor(aggregated_likelihoods) - llh_shift
        # print(aggregated_likelihoods)
        # print(aggregated_conf)
...
with open(f'{config.output_dir}/sequences/{run_name}/{args.generation_model.split("/")[-1]}_generations.pkl', 'rb') as infile:
    ...
    for sample in sequences:
        confidence_list.append(compute_conf(sample['confidence']))
...
with open(f'{config.output_dir}/sequences/{run_name}/{args.generation_model.split("/")[-1]}_generations_{args.evaluation_model.split("/")[-1]}_likelihoods.pkl',
          'rb') as infile:
    sequences = pickle.load(infile)
    print('sequences keys: ', sequences[0].keys())
    print('number of sequences: ', len(sequences))
    print(sequences[15]['average_neg_log_likelihoods'])
    mask = sequences[4]['generations'] != 1
    indices = torch.nonzero(mask, as_tuple=True)
    print('sequences', indices)
    list_of_results.append((args.evaluation_model, sequences))

overall_results = get_overall_log_likelihoods(list_of_results)
# print('Semantic set ids: ', overall_results['semantic_set_ids'])
# print('list of results: ', list_of_results)
# print('Overall results keys: ', overall_results)
...
unnormalised_entropy_over_concepts = get_predictive_entropy_over_concepts(-overall_results['neg_log_likelihoods'],
                                                                          overall_results['semantic_set_ids'])
print('predictive_entropy_over_concepts', predictive_entropy_over_concepts)
print('unnormalised_entropy_over_concepts', unnormalised_entropy_over_concepts)

pred_entropy = predictive_entropy_over_concepts.tolist()
unnorm_entropy = unnormalised_entropy_over_concepts.tolist()

df = pd.DataFrame({
    "predictive_entropy_over_concepts": pred_entropy,
    "unnormalised_entropy_over_concepts": unnorm_entropy,
    "confidence_list": confidence_list
})

df.to_csv(f'{config.output_dir}/sequences/{run_name}/{args.generation_model.split("/")[-1]}_conf_uncertainty.csv', index=False)
...
# print('semantic_predictive_entropy_on_subsets', semantic_predictive_entropy_on_subsets)
...
#     print('sequences keys: ', sequences[0].keys())
#     print('number of sequences: ', len(sequences))
#     print(sequences[4]['generations'])
#     mask = sequences[4]['generations'] != 1
#     indices = torch.nonzero(mask, as_tuple=True)
#     print('sequences', indices)
# 
# run_name = "decent-field-17"
# model='meta-llama/Llama-2-7b-hf'
# with open(f'{config.output_dir}/sequences/{run_name}/{model.split("/")[-1]}_generations_{model.split("/")[-1]}_likelihoods.pkl', 'rb') as infile:
#     sequences = pickle.load(infile)
# 
# sequence = sequences[4]
# print(sequence.keys())
# print(sequence['generations'])